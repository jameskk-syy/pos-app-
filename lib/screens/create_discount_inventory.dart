...
import '../domain/responses/item_group.dart';

class CreateDiscountRulePage extends StatefulWidget {
  const CreateDiscountRulePage({super.key});
...
class _CreateDiscountRulePageState extends State<CreateDiscountRulePage> {
  ...
  void _createDiscountRule() {
    ...
    context.read<InventoryBloc>().add(
      CreateDiscountRule(request: request),
    );
  }
        ...
        BlocListener<ProductsBloc, ProductsState>(
          listener: (context, state) {
            if (state is ProductsItemGroupsStateSuccess) {
              ...
              setState(() {
                items = state.productResponseSimple.products
        ...
        BlocListener<InventoryBloc, InventoryState>(
          listener: (context, state) {
            if (state is CreateDiscountRuleLoading) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('Discount rule created successfully: $ruleName'),
                            ...
                            children: [
                              ...
                              SegmentedButton<String>(
                                segments: const [
                                  ...
                                  ButtonSegment<String>(
                                    value: 'ItemGroup',
                                    label: Text('Item Group', style:  TextStyle(fontSize: 12)  ),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ElevatedButton(
                              onPressed: isSubmitting ? null : _createDiscountRule,
                              style: ElevatedButton.styleFrom(
                                backgroundColor: isSubmitting ? Colors.grey[400] : const Color(0xFF1976F3),
                                padding: const EdgeInsets.symmetric(
                                  vertical: 16,
                                ),
                                shape: RoundedRectangleBorder(
