...
class _CheckoutWizardPageState extends State<CheckoutWizardPage> {
      ...
      if (state is CrmStateSuccess) {
        ...
        setState(() {
          ...
          if (selectedCustomerId == null) {
            _selectedCustomer = walkInCustomer;
  ...
  void _selectCustomer(Customer customer) {
    setState(() {
      ...
      selectedCustomerName = customer.customerName.isNotEmpty
  ...
  void _calculateChange() {
    final paidText = _amountPaidController.text;
    if (paidText.isEmpty) {
      setState(() {
        _changeAmount = 0.0;
      });
    ...
    final paidAmount = double.tryParse(paidText) ?? 0.0;
                    ...
                    .map<DropdownMenuItem<String>>((method) {
                      return DropdownMenuItem<String>(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              _formatPaymentName(method['name'] as String),
                              style: const TextStyle(fontSize: 14),
                            ),
                            if (method['type'] != null)
                              Text(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixText:
                        'KES', // Defaulting to KES as seen in project context or currency placeholder
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'KES ${_changeAmount.toStringAsFixed(2)}',
                          style: TextStyle(
                            fontSize: MediaQuery.of(context).size.width < 600
                                ? 24
                                : 32,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        _formatPaymentName(selectedPayment),
                        style: const TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w600,
                        ),
    ...
    if (_currentStep == 2) {
      if (_isCreditPayment()) {
        ...
        if (customer == null ||
            ...
            customer.availableCredit < widget.total) {
  ...
  bool _isCreditPayment() {
    ...
    final normalizedName = selectedPayment.toLowerCase();
    if (normalizedName.contains('credit') ||
        normalizedName.contains('account')) {
      return true;
    }

    try {
      final method = paymentMethodsFromApi.firstWhere(
        (m) => m['name'] == selectedPayment,
      );
      final type = method['type']?.toString().toLowerCase() ?? '';
      return type == 'credit' || type == 'account';
    } catch (_) {
      return false;
    }
  }
  ...
  void _showCreditLimitWarning(double limit, double available, double total) {
    showDialog(
      context: context,
          ...
          return Dialog(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.zero, // Square as requested
              ),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  const SizedBox(height: 16),
                  const Text(
                    'This transaction cannot proceed on credit for the following reason:',
                    style: TextStyle(fontSize: 14, color: Colors.grey),
                  ),
                  const SizedBox(height: 16),
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildCreditWarningItem(
                          'Credit Limit:',
                          '${limit.toStringAsFixed(2)} KES',
                        ),
                        const SizedBox(height: 8),
                        _buildCreditWarningItem(
                          'Available Credit:',
                          '${available.toStringAsFixed(2)} KES',
                          isCritical: available < total,
                        ),
                        const SizedBox(height: 8),
                        _buildCreditWarningItem(
                          'Current Total:',
                          '${total.toStringAsFixed(2)} KES',
                        ),
                      ],
                    ),
                  ),
                  ...
                  Text(
                    limit <= 0
                        ? 'This customer has no credit facility enabled.'
                        : 'The total amount exceeds the available credit limit.',
                    style: TextStyle(
                      fontSize: 13,
  ...
  Widget _buildCreditWarningItem(
    String label,
    String value, {
