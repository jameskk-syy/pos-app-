...
class _AssignLoyaltyDialogState extends State<AssignLoyaltyDialog> {
  ...
  bool _isProgramExpired(dynamic program) {
    final toDate = (program is LoyaltyProgram) 
        ? program.toDate 
        : (program.toDate ?? '');
        
    if (toDate.isEmpty) return false;

    try {
      final expiryDate = DateTime.parse(toDate);
      ...
      listener: (context, state) {
        if (state is GetLoyaltyProgramsLoaded) {
          ...
          final programs = state.response.message.programs
              .where((program) => !_isProgramExpired(program))
              .toList();
