...
class ProductItem {
  ...
  factory ProductItem.fromJson(Map<String, dynamic> json) {
    return ProductItem(
      ...
      stockUom: json['stock_uom']?.toString() ?? 'NOS',
      standardRate: (json['standard_rate'] as num?)?.toDouble() ?? 0.0,
      isStockItem: (json['is_stock_item'] as num?)?.toInt() ?? 0,
      isSalesItem: (json['is_sales_item'] as num?)?.toInt() ?? 0,
      isPurchaseItem: (json['is_purchase_item'] as num?)?.toInt() ?? 0,
...
class ProductResponseSimple {
  ...
  factory ProductResponseSimple.fromJson(dynamic json) {
    ...
    return ProductResponseSimple(
      ...
      pagination: pagination,
      priceList:
          (messageMap['price_list'] ?? jsonMap['price_list'] ?? '') as String,
    );
  }
